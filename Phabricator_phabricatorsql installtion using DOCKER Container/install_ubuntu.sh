...
#  sudo add-apt-repository ppa:ondrej/php  || failed
fi
ROOT=`pwd`
...
apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y php7.1\
  git apache2 libapache2-mod-php7.1 php7.1-common\
  php7.1-curl php7.1-gd php7.1-mysql php7.1-mbstring\
  || failed
